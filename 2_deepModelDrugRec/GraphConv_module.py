...
class Aggregator(nn.Module):
    ...
    def forward(self, entity_emb, user_emb, latent_emb,
        ...
        edge_relation_emb = weight[edge_type]  # exclude code VS leaf_kg interact,
        neigh_relation_emb = entity_emb[tail] * edge_relation_emb  # [-1, channel]
        entity_agg = scatter_mean(src=neigh_relation_emb, index=head, dim_size=n_entities, dim=0) # [node, channel]
        ...
        """user aggregate"""
        # user_intact_agg = torch.sparse.mm(interact_mat, entity_emb)  # [n_users, channel]
        user_intact_agg = torch.sparse.mm(interact_mat, entity_agg)  # [n_users, channel]
        disen_weight = torch.mm(nn.Softmax(dim=-1)(disen_weight_att),
                                weight).expand(n_users, n_factors, channel) # nn.Softmax(dim=-1)(disen_weight_att).shape=[n_factors, rel] weight.shape=(rel, channel)
        user_agg = user_intact_agg * (disen_weight * score).sum(dim=1) + user_intact_agg  # [n_users, channel]

        return entity_agg, user_agg, user_intact_agg
...
class GraphConv(nn.Module):
    ...
    def forward(self, user_emb, entity_emb, latent_emb, edge_index, edge_type,
        ...
        # print("entity_emb:", entity_emb)
        # print("latent_emb:", latent_emb)
        # print("edge_index:", edge_index)
        # print("edge_type:", edge_type)
        # print("interact_mat:", interact_mat)
        # print("mess_dropout:", mess_dropout)
        # print("node_dropout:", node_dropout)
        ...
        for i in range(len(self.convs)):
            entity_emb, user_emb, user_intact_emb = self.convs[i](entity_emb, user_emb, latent_emb,
                                                 edge_index, edge_type, interact_mat,
        ...
        return entity_res_emb, user_res_emb, cor, user_intact_emb
