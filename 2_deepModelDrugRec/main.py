...
from recommend import eval, test, test_addPath
...
parser.add_argument('--threshold', type=float, default=0.5, help='the threshold of prediction')
...
parser.add_argument('--codeMI_regularity', type=float, default=1,
                    help='regularization weight for MI loss between original and enhanced code embedding')
...
def test_model(model, resume_path, device, test_dataloader, diag_voc, pro_voc, med_voc, voc_size, TOKENS, args):
    ...
    tic = time.time()
    smm_record, ja, prauc, precision, recall, f1, med_num = test_addPath(model, resume_path, test_dataloader, diag_voc,
                                                                         pro_voc, med_voc, voc_size, 0, device, TOKENS,
                                                                         args)
    result = []
    for _ in range(10):
        data_num = len(ja)
...
def main(args):
    data_path = "../dataset/mimic-iii/records_final.pkl"
    voc_path = "../dataset/mimic-iii/voc_final.pkl"
    kg_voc_path = '../dataset/mimic-iii/code2cui_voc.pkl'
    kg_path = '../dataset/mimic-iii/kg.pkl'
    ...
    diag2cui, proc2cui, med2cui, kg2idx, rel2idx = kg_voc['diag2cui'], kg_voc['proc2cui'], kg_voc['med2cui'], kg_voc[
        'kg2id'], kg_voc['rel2id']
    print("UMLS triple num:{}".format(len(kg)))
    print("UMLS node num: {}".format(len(list(kg2idx.kg2idx.keys()))))

    n_nodes, n_entities, n_leaf_nodes, kg_graph, adj_mat_list, norm_mat_list, mean_mat_list = KGIN_data_loader(kg,
                                                                                                               kg_voc,
                                                                                                               voc)
    ...
    # data_train = data[:int(split_point*0.05)]
    ...
    model = IntentKG(voc_size, ehr_adj, n_nodes, args.n_intents, n_relations, n_entities, kg_graph, \
                     ...
                     emb_dim=args.emb_dim, context_hops=args.context_hops, ind=args.ind,
                     node_dropout_rate=args.node_dropout_rate, \
                     mess_dropout_rate=args.mess_dropout_rate, device=device, dim_hidden=args.emb_dim, ln=args.ln,
                     kgloss_alpha=args.kgloss, num_heads=args.num_heads)
    ...
    for epoch in range(EPOCH):
        tic = time.time()
        print(
            '\nepoch {} seed : {} bs:{} model : {} --------------------------'.format(epoch, args.seed, args.batch_size,
                                                                                      args.model_name))
        model.train()
        for idx, data in enumerate(train_dataloader):
            ...
            # print(diseases)
            # print("---------------------")
            output_logits, _, code_mi_loss, intent_cor = model(diseases, procedures, medications, d_mask_matrix,
                                                               p_mask_matrix, m_mask_matrix, seq_length, dec_disease,
                                                               stay_disease, dec_disease_mask, stay_disease_mask,
        ...
        saved_path = os.path.join("./saved_results/mimic-iii/{}".format(args.model_name),
                                  'Bs_{}'.format(args.batch_size),
                                  'lr_{}_hs_{}_ln_{}_kg_{}_intent_{}_ind_{}_CodeMI_{}_InCor_{}_ndr_{}_nhead{}'.format(
                                      args.lr, args.emb_dim, int(args.ln), args.kgloss, args.n_intents, args.ind,
                                      args.codeMI_regularity, args.sim_regularity, args.node_dropout_rate,
        ...
        if best_ja < mean[1]:
            ...
            test_model(model, best_saved_model, device, test_dataloader, diag_voc, pro_voc, med_voc, voc_size, TOKENS,
                       args)
