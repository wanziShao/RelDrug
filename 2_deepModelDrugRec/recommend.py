...
def test_addPath(model, resume_path, test_dataloader, diag_voc, pro_voc, med_voc, voc_size, epoch, device, TOKENS, args):
    ...
    # Prepare output file
    result_dir = os.path.dirname(os.path.dirname(resume_path))
    pred_file_path = os.path.join(result_dir, "rec-real-prob-all.json")
    pred_data = []
    ...
    for idx, data in enumerate(test_dataloader):
        ...
        # Ground-truth and predicted per visit
        actual_per_visit = target_list[0]  # list of arrays
        out_per_visit = []
        for prob_list in predictions[0]:
            out_per_visit.append(np.where(prob_list > args.threshold)[0])
        ...
        for v_idx, actual in enumerate(actual_per_visit):
            prob_list = predictions[0][v_idx]
            # Get all medication IDs and their probabilities
            med_probs = [(int(med_id), float(prob)) for med_id, prob in enumerate(prob_list)]
            # Sort by probability descending (optional)
            med_probs.sort(key=lambda x: x[1], reverse=True)
        ...
        pred_data.append(patient_data)

        # Continue existing metric calculations
        y_gt, y_pred, y_pred_label, y_pred_prob = [], [], [], []
        label_history = []
        for label, prob_list in zip(actual_per_visit, predictions[0]):
            label_history.extend(label)
            y_gt_tmp = np.zeros(voc_size[2])
            y_gt_tmp[label] = 1
            y_gt.append(y_gt_tmp)
        ...
        smm_record.append(y_pred_label)
        for i in range(min(len(actual_per_visit), 5)):
            single_ja, single_auc, single_p, single_r, single_f1 = sequence_metric(
                np.array([y_gt[i]]),
                np.array([y_pred[i]]),
            ...
            smm_record_by_visit[i].append([y_pred_label[i]])

        all_pred_list[f'p_{idx}'] = out_per_visit
        all_label_list[f'p_{idx}'] = actual_per_visit
        ...
        avg_r.append(adm_avg_r)
        avg_f1.append(adm_avg_f1)
        llprint(f'\rtest step: {idx+1} / {len(test_dataloader)}')

        # Record by visit
        record_ja[f'v_{seq_length.item()}'].append(adm_ja)
        record_prauc[f'v_{seq_length.item()}'].append(adm_prauc)
        record_p[f'v_{seq_length.item()}'].append(adm_avg_p)
        record_r[f'v_{seq_length.item()}'].append(adm_avg_r)
        record_f1[f'v_{seq_length.item()}'].append(adm_avg_f1)
    ...
    # Print final visit-level metrics
